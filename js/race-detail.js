...
(async function () {
    ...
    try {
        ...
        renderRaceActions(raceInfo);
        renderCircuitShowcase(raceInfo);
        renderRaceInfo(raceInfo, race);
        renderTrackIntel(raceInfo, race);
        saveRecentRace(raceInfo);
    ...
    function renderRaceHeader(race) {
        ...
        // Fetch and add circuit image
        const circuitUrl = race.Circuit?.url;
        const circuitName = race.Circuit?.circuitName;
        if (circuitUrl || circuitName) {
            fetchCircuitImage(circuitUrl, circuitName).then(() => {
                const img = circuitImageHTML(circuitUrl, 160, circuitName);
                if (img) {
                    const header = document.getElementById('race-header');
                    if (header) {
                        header.style.position = 'relative';
                        header.querySelector('.race-header-circuit')?.remove();
                        const imgDiv = document.createElement('div');
                        imgDiv.className = 'race-header-circuit race-header-circuit-preview';
                        imgDiv.innerHTML = `
                            <div class="race-header-circuit-label">Track Snapshot</div>
                            ${img}
                            <button type="button" class="race-header-circuit-jump">Open Full Layout</button>`;

                        const jumpToFullLayout = () => {
                            const showcase = document.getElementById('race-circuit-showcase');
                            if (!showcase) return;
                            showcase.scrollIntoView({ behavior: 'smooth', block: 'start' });
                            showcase.classList.add('race-circuit-focus');
                            setTimeout(() => showcase.classList.remove('race-circuit-focus'), 1200);
                        };

                        imgDiv.addEventListener('click', jumpToFullLayout);
                        imgDiv.querySelector('.race-header-circuit-jump')?.addEventListener('click', (event) => {
    ...
    function renderRaceActions(race) {
        let bar = document.getElementById('race-action-bar');
        if (!bar) {
            bar = document.createElement('div');
            bar.id = 'race-action-bar';
            bar.className = 'race-action-bar';
            const header = document.getElementById('race-header');
            header?.insertAdjacentElement('afterend', bar);
        }

        const shareUrl = new URL(window.location.href);
        shareUrl.searchParams.set('season', String(race.season || 'current'));
        shareUrl.searchParams.set('round', String(race.round || ''));
        const link = shareUrl.toString();
        bar.innerHTML = `
            <button id="race-copy-link" class="ui-btn" type="button">Copy Race Link</button>
            <a href="schedule.html" class="ui-btn">Back to Calendar</a>
            <a href="schedule.html#races-container" class="ui-btn">More Races</a>`;

        const copyBtn = bar.querySelector('#race-copy-link');
        copyBtn?.addEventListener('click', async () => {
            try {
                ...
                setTimeout(() => { copyBtn.textContent = 'Copy Race Link'; }, 1200);
            } catch (e) {
    ...
    function renderCircuitShowcase(race) {
        const circuitUrl = race.Circuit?.url;
        const circuitName = race.Circuit?.circuitName;
        ...
        if (!section) {
            ...
            section.className = 'section race-circuit-showcase';
        ...
        fetchCircuitImage(circuitUrl, circuitName).then(() => {
            const circuitMarkup = circuitImageHTML(circuitUrl, 360, circuitName);
            if (!circuitMarkup) {
                section.style.display = 'none';
                return;
            }
            ...
            section.innerHTML = `
                <div class="section-header">
                    <h2>Circuit <span class="text-red">Layout</span></h2>
                </div>
                <div class="race-circuit-wrap">
                    <div class="race-circuit-main" id="race-circuit-main">${circuitMarkup}</div>
                    <div class="race-circuit-meta">
                        <span class="race-circuit-chip">${race.Circuit?.circuitName || 'Circuit'}</span>
                        <span class="race-circuit-chip">${race.Circuit?.Location?.country || 'Unknown Country'}</span>
                        <span class="race-circuit-chip">Round ${race.round || '?'}</span>
                    </div>
                    <button id="expand-circuit-btn" class="ui-btn" type="button">Expand Circuit</button>
                </div>`;

            const main = section.querySelector('#race-circuit-main');
            const expandBtn = section.querySelector('#expand-circuit-btn');
            const lightbox = ensureCircuitLightbox();
            const lightboxImg = lightbox.querySelector('#circuit-lightbox-img');

            const openLightbox = () => {
                const baseUrl = getCircuitImageUrl(circuitUrl, circuitName);
                if (!baseUrl || !lightboxImg) return;
                const largeUrl = String(baseUrl).replace(/\/\d+px-/, '/1280px-');
                lightboxImg.src = largeUrl;
                lightboxImg.alt = `${circuitName || 'Circuit'} layout`;
    ...
    function renderTireStrategy(stints) {
        ...
        container.innerHTML = `
            <div style="background:var(--f1-surface);border:1px solid var(--f1-border);border-radius:var(--radius-lg);padding:var(--space-lg);">
