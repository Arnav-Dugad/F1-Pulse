...
(async function () {
    ...
    let enrichedDrivers = {};
    try {
        enrichedDrivers = await Promise.race([
            loadEnrichedDrivers(),
    ...
    function renderQuickStats(lastRace, standings, schedule, enriched, statusData) {
        ...
        const totalRaces = Array.isArray(schedule) ? schedule.length : null;
        const completedRaces = Array.isArray(schedule) ? schedule.filter(r => {
    ...
    function renderSeasonProgress(schedule) {
        ...
        const orderedSchedule = [...schedule].sort((a, b) => (parseInt(a?.round, 10) || 0) - (parseInt(b?.round, 10) || 0));

        const now = new Date();
        const total = orderedSchedule.length;
        const completed = orderedSchedule.filter(r => new Date(r.date) < now).length;
            ...
            <div class="season-progress-bar">
                ...
                <div class="season-progress-track">
                    ...
                    ${orderedSchedule.map((r, i) => {
                        ...
                        const isNext = !isPast && (i === 0 || new Date(orderedSchedule[i - 1]?.date) < now);
    ...
    setInterval(async () => {
        try {
            ...
            if (ds.status === 'fulfilled' && ds.value) {
                renderTopDrivers(ds.value, enrichedDrivers);
                renderChampionshipBattle(ds.value, enrichedDrivers);
                renderTeammateH2H(ds.value, enrichedDrivers);
            }
            if (cs.status === 'fulfilled' && cs.value) {
                renderTopConstructors(cs.value, enrichedDrivers);
                renderConstructorBattle(cs.value);
            }
        } catch (e) {
            // Silent refresh failure; page keeps current data.
        }
    }, 5 * 60 * 1000);
