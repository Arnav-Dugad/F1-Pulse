...
function ensureScheduleFeatureControls() {
    ...
    controls.innerHTML = `
        <div class="schedule-controls-grid">
            <label class="schedule-control-field">
                <span class="schedule-control-label">Search Circuits</span>
                <input id="schedule-search" class="schedule-search ui-input" type="search" placeholder="Race, circuit, country...">
            </label>
    ...
    const searchInput = controls.querySelector('#schedule-search');
    ...
    searchInput?.addEventListener('input', (e) => {
        currentSearchQuery = String(e.target.value || '').trim().toLowerCase();
        saveSchedulePrefs({ searchQuery: currentSearchQuery });
...
function applyScheduleFilters(races) {
    ...
    if (currentSearchQuery) {
        filtered = filtered.filter((r) => {
            const raceName = String(r.raceName || '').toLowerCase();
            const circuitName = String(r.Circuit?.circuitName || '').toLowerCase();
            const country = String(r.Circuit?.Location?.country || '').toLowerCase();
            const locality = String(r.Circuit?.Location?.locality || '').toLowerCase();
            return raceName.includes(currentSearchQuery)
                || circuitName.includes(currentSearchQuery)
                || country.includes(currentSearchQuery)
                || locality.includes(currentSearchQuery);
        });
    }
...
function renderRaces() {
    ...
    const viewedSet = new Set(getRecentRaces().map((x) => `${x.season}-${x.round}`));
    ...
    container.innerHTML = races.map((r, i) => {
        ...
        const countdownBadge = !isPast
            ...
            : '';
        const isViewed = viewedSet.has(`${r.season}-${r.round}`);
        ...
        <a href="race-detail.html?season=${r.season}&round=${r.round}"${nextIdAttr} class="race-card ${isPast ? 'past' : ''} ${isNext ? 'next' : ''}">
            ...
            <div class="race-card-body">
                ...
                ${isViewed ? '<div style="margin-top:8px"><span class="badge">Viewed</span></div>' : ''}
...
(async function () {
    ...
    currentSearchQuery = String(prefs.searchQuery || '').toLowerCase();
