...
(async function () {
    ...
    try {
        ...
        const factIcon = {
            'Full Team Name': SVG_ICONS.flagChequered,
            'Base': SVG_ICONS.flagBlue,
            'Team Chief': SVG_ICONS.trophy,
            'Technical Chief': SVG_ICONS.road,
            'Chassis': SVG_ICONS.wind,
            'Power Unit': SVG_ICONS.droplet,
            'Reserve Driver': SVG_ICONS.medalSilver,
            'First Team Entry': SVG_ICONS.calendar,
        };
        ...
        ].map(([label, value]) => `<article class="team-profile-fact"><span><span class="profile-inline-icon">${factIcon[label] || SVG_ICONS.flagChequered}</span>${teamProfileEsc(label)}</span><strong>${teamProfileEsc(value || 'Updating')}</strong></article>`).join('');
        ...
        main.innerHTML = `
            ...
            <section class="team-drivers-showcase" style="--team:${teamColor};--team-top:${teamProfileEsc(gradient.top)};--team-mid:${teamProfileEsc(gradient.mid)};--team-bottom:${teamProfileEsc(gradient.bottom)}">
                <header class="team-drivers-showcase-head">
                    <h3><span class="profile-inline-icon">${SVG_ICONS.flagChequered}</span>Drivers</h3>
                </header>
            ...
            <section class="team-bio-shell" style="--team:${teamColor};--team-top:${teamProfileEsc(gradient.top)};--team-mid:${teamProfileEsc(gradient.mid)};--team-bottom:${teamProfileEsc(gradient.bottom)}">
                <header class="team-bio-header">
                    <h3><span class="profile-inline-icon">${SVG_ICONS.road}</span>Team Profile</h3>
