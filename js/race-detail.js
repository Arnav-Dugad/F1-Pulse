...
(async function () {
    ...
    try {
        ...
        const raceStartISO = raceInfo?.time
            ? `${raceInfo.date}T${raceInfo.time}`
            : `${raceInfo?.date || ''}T14:00:00Z`;
        const raceStartMs = Date.parse(raceStartISO);
        const isUpcomingRace = !hasRaceResults && Number.isFinite(raceStartMs) && raceStartMs > Date.now();

        renderRaceHeader(raceInfo);
        renderRaceActions();
        renderRaceCountdown(raceInfo, {
            includeWeekendSchedule: isUpcomingRace,
            includeUpcomingNotice: isUpcomingRace,
        });
        ...
        if (hasRaceResults || hasQualResults || hasSprintResults) {
            ...
            // Load OpenF1 data in background for weather, stints, race control, and strategy analysis
            loadOpenF1Data(raceInfo, hasRaceResults, race, pitStops);
            reorderRaceDetailSections();
        } else {
            raceResultsEl.innerHTML = '';
            renderUpcomingInfo(raceInfo, { compact: isUpcomingRace });
        }
    } catch (err) {
    ...
    function renderRaceCountdown(race, options = {}) {
        const includeWeekendSchedule = !!options.includeWeekendSchedule;
        const includeUpcomingNotice = !!options.includeUpcomingNotice;
        ...
        const sessions = includeWeekendSchedule ? getWeekendSessions(race) : [];
        const weekendScheduleHTML = sessions.length
            ? `<div class="race-weekend-schedule"><h4>Weekend Schedule</h4>${sessions.map((s) => `
                <div class="race-weekend-row">
                    <span class="race-weekend-title">${s.name}</span>
                    <span class="race-weekend-time">${formatDate(s.date)}${s.time ? ` · ${formatTimeOnly(s.time)} UTC` : ''}</span>
                </div>
            `).join('')}</div>`
            : '';
        const upcomingNoticeHTML = includeUpcomingNotice
            ? `<div class="race-upcoming-top-note">
                <h3>Race has not taken place yet</h3>
                <p>Countdown and weekend schedule are shown at the top of this page.</p>
            </div>`
            : '';
    ...
    function hideTelemetryMiniPanel() {
        if (telemetryRefreshTimer) {
            clearInterval(telemetryRefreshTimer);
            telemetryRefreshTimer = null;
        }
    ...
    function renderUpcomingInfo(race, options = {}) {
        ...
        const raceStartMs = Date.parse(raceDateTime);
        const isUpcoming = Number.isFinite(raceStartMs) && raceStartMs > Date.now();

        const title = isUpcoming
            ? (compact ? 'Results pending' : 'Race has not taken place yet')
            : 'Race data not available';
        const desc = isUpcoming
            ? (compact
                ? 'Results will appear here after the race.'
                : 'Results will appear here after the race.')
            : 'Official session results are not available for this race yet.';

        upcomingEl.innerHTML = `
            <div class="race-upcoming-panel" style="text-align:center;padding:${compact ? '28px 0 10px' : '48px 0'};">
                <div class="race-upcoming-icon" style="margin-bottom:16px">${SVG_ICONS.flagChequered}</div>
                <h3 style="margin-bottom:8px">${title}</h3>
                <p style="color:var(--f1-gray);margin-bottom:0;">${desc}</p>
            </div>`;
