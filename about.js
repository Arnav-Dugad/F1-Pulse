...
(async function () {
    ...
    const driverSection = document.getElementById('driver-bios-section');
    const teamSection = document.getElementById('team-profiles-section');
    const teamFilter = document.getElementById('about-team-filter');
    const resultsMeta = document.getElementById('about-results-meta');
    const viewButtons = Array.from(document.querySelectorAll('.about-view-btn'));
    ...
    try {
        ...
        buildTeamFilterOptions(state.teams);
        installControls();
        renderFiltered();
    } catch (err) {
    ...
    function installControls() {
        if (teamFilter) {
            teamFilter.addEventListener('change', (e) => {
                state.team = String(e.target?.value || 'all');
                renderFiltered();
            });
        }
    ...
    function syncViewButtons() {
        viewButtons.forEach((btn) => {
            const active = (btn.dataset.view || 'all') === state.view;
            btn.classList.toggle('active', active);
            btn.setAttribute('aria-pressed', active ? 'true' : 'false');
        });
    }

    function buildTeamFilterOptions(teams) {
        ...
        (teams || []).forEach((item) => {
            const raw = item?.Constructor?.constructorId;
            const teamId = resolveTeamId(raw) || raw || '';
            const teamName = getTeamName(raw);
            if (!teamId || existing.has(teamId)) return;
            existing.add(teamId);
            options.push(`<option value="${aboutEsc(teamId)}">${aboutEsc(teamName)}</option>`);
        });
    ...
    function filterDrivers() {
        return (state.drivers || []).filter((item) => {
            const d = item?.Driver || {};
            const rawTeamId = item?.Constructors?.[0]?.constructorId;
            const teamId = resolveTeamId(rawTeamId) || rawTeamId || '';
            if (state.team !== 'all' && teamId !== state.team) return false;
            return true;
        });
    }
    ...
    function filterTeams() {
        return (state.teams || []).filter((item) => {
            const rawTeamId = item?.Constructor?.constructorId;
            const teamId = resolveTeamId(rawTeamId) || rawTeamId || '';
            if (state.team !== 'all' && teamId !== state.team) return false;
            return true;
        });
    ...
    function renderFiltered() {
        ...
        renderHeroMetrics(state.drivers, state.teams, drivers, teams);
        renderDriverTiles(drivers, state.enriched || {});
        renderTeamTiles(teams);
        syncViewButtons();

        if (driverSection) {
            driverSection.style.display = state.view === 'teams' ? 'none' : '';
        }

        if (teamSection) {
            teamSection.style.display = state.view === 'drivers' ? 'none' : '';
        }

        if (resultsMeta) {
            const filters = [];
            if (state.team !== 'all') filters.push(`team ${getTeamName(state.team)}`);
            const filterText = filters.length ? `Filtered by ${filters.join(' and ')}` : 'No filters';
            resultsMeta.textContent = `${drivers.length}/${totalDrivers} drivers and ${teams.length}/${totalTeams} teams. ${filterText}.`;
        }
    }
