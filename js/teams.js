...
(async function () {
    ...
    function getDisplayStandings() {
        return [...state.constructorStandings].sort((a, b) => (parseFloat(b.points) || 0) - (parseFloat(a.points) || 0));
    }
    ...
    function renderCurrentView() {
        ...
        renderInsights(state.constructorStandings, displayStandings);
        renderPerformanceCharts(state.constructorStandings, displayStandings);
        renderTeamFormTrends(displayStandings);
        renderTeamCards(displayStandings, state.teamDrivers, state.constructorStandings);
        renderTable(displayStandings, state.constructorStandings);
    }
    ...
    function renderTeamCards(standings, teamDrivers, fullStandings) {
        if (!standings?.length) {
            container.innerHTML = '<div class="section" style="text-align:center;padding:24px;color:var(--f1-gray)">No teams available right now.</div>';
            return;
        }
    ...
    function renderTable(standings, fullStandings) {
        ...
        if (!standings?.length) {
            tableContainer.innerHTML = '<div class="section" style="margin:0;text-align:center;padding:20px;color:var(--f1-gray)">No table rows available right now.</div>';
            return;
        }
    ...
    function ensureTeamsEnhancementUI(targetContainer) {
        ...
        if (!wrapper) {
            ...
            wrapper.innerHTML = `
                ...
                <div class="section-header" style="margin-top:var(--space-lg)"><h2 data-iconized="true">${teamIcon(TEAM_UI_ICONS.filter)}Team Form Momentum</h2></div>
