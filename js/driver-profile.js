...
(async function () {
    ...
    try {
        ...
        if (subtitle) {
            subtitle.textContent = `${displayName} · ${teamName} biography`;
        }
        ...
        main.innerHTML = `
            <section class="driver-brand-hero" style="--team:${teamColor};--team-top:${driverProfileEsc(gradient.top)};--team-mid:${driverProfileEsc(gradient.mid)};--team-bottom:${driverProfileEsc(gradient.bottom)}">
                <div class="driver-brand-left">
                    ...
                    <p class="driver-brand-meta">${flag} <span>${driverProfileEsc(country || 'Unknown')}</span><span class="dot">|</span><span>${driverProfileEsc(teamName)}</span><span class="dot">|</span><span>${driverProfileEsc(String(driverNumber))}</span></p>
                </div>
